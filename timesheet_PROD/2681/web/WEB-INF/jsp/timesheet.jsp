        ...
        <td width="10%" align="left" style="padding-left:10px;">
            <c:choose>
                <c:when test="${previous_month != '' && previous_year != ''}">
                    <a href="javascript:;" onClick="loadTimesheetPrevious(${previous_month},${previous_year},'${status}')"><h4><< Previous</h4></a>
                </c:when>
                <c:otherwise>
                    <h4><< Previous</h4>
                </c:otherwise>
        ...
        <td width="10%" align="right" style="padding-right:10px;">
            <c:choose>
                <c:when test="${next_month != '' && next_year != ''}">
                    <a href="javascript:;" onClick="loadTimesheetNext(${next_month},${next_year},'${status}')"><h4>Next >></h4></a>
                </c:when>
                <c:otherwise>
                    <h4>Next >></h4>
                </c:otherwise>
...
<table class="roleWorkLocTable" style="width: 98.75%;margin: 10px 5px 0px 5px; border: 1px solid #BDC9D1;background: url(images/box-strip.jpg) repeat-x scroll top center;border-radius: 5px; background-size: 100% 100%;">
    <tbody>
    </tbody>
</table>
...
<table class="tableStructure" border="0">
    <tr class="headerCenter">
        ...
        <th width="14%">Project<span style="color:red; margin-left:3px;">*</span></th>
        <th width="10%">Shift</th>
        <th width="10%">Role</th>
        <th width="10%">Phase</th>
        <th width="10%">Task</th>
        <th width="5%">Office Hours (HH:MM)</th>
        <th width="5%">Attendance Hours (HH:MM)</th>
        <th width="5%">Available Hours (HH:MM)</th>
        <th width="10%">Timesheet Hours (HH:MM)<span style="color:red;margin-left:3px;">*</span></th>
        <th width="8%">Regularization</th>
    ...
    <c:forEach items="${timeSheetObj}" var="Resultset" varStatus="index">
        ...
        <td style="text-align:center;">
            <input type="hidden" value="${office_start_time}"/>
            <select ${disabledField} name="res_shift" id="shift_${index.count}" style="width:100%">
                <c:if test="${disabledField=='disabled'}">
                ...
                <c:if test="${disabledField!='disabled'}">
                    <c:forEach  items="${shift_list}" var="shiftList" >
                        <option title="${shiftList.configuration_value}" ${(office_start_time == shiftList.configuration_key)?'selected':''} value="${shiftList.configuration_key}">${shiftList.configuration_value}</option>
                    </c:forEach>
                </c:if>
            </select>
        </td>
        ...
        <td>
            ...
            <select ${disabledField} name="res_shift" id="regularize_${index.count}" style="width:75px">
        ...
        <td style="text-align:center;">
            ...
            <c:if test="${Resultset.status =='r'}">
                <span id="viewComments_${index.count}" style ="color:red;cursor:pointer;">RM/PM Remarks</span>
            </c:if>
        </td>
...
<c:choose>
    <c:when test="${fn:length(timeSheetObj) != 0}">
        <tr class="headerCenter">
            <th colspan="10" align="right">Total Hours</th>
            <td style="text-align:center;">
...
<c:if test="${fn:length(timeSheetObj) != 0}">
    <div class="actionToSave" style="text-align:center;margin:0 0 20px 20px;" >
        ...
        <input type="button" name="buttonSave" style="cursor: pointer;" id="buttonSave" value="Save" class="qualitysave"/>
        <input type="button" name="buttonSubmit" style="cursor: pointer;" id="buttonSubmit" value="Submit" class="submitbutton"/>    
