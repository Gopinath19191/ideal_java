...
public class SendMail {
    ...
    public boolean smtpMailAttachment(final String mailTo, final String mailSubject, final String mailMessage, final String mailCC,final String attachment) throws Exception {
        ...
        if (attachment != null) {
            if(attachment!=null){
                ...
                System.out.println("file name "+attachment);
            }
        }
        message.setContent(multipart);
        InternetAddress[] toList;
    ...
    public boolean smtpMailAttachmentIdCard(final String mailTo, final String mailSubject, final String mailMessage, final String mailCC, 
            final String attachment, final String id_photo) throws Exception {
        System.out.println("emailProps = " + emailProps);
        final Session mailSession = Session.getInstance(emailProps,new javax.mail.Authenticator() {
                @Override
                protected PasswordAuthentication getPasswordAuthentication()
        ...
        if (attachment != null) {
            if(attachment!=null){
                //addAttachmentIdCard(multipart,attachment);
                addAttachmentIdCard(multipart,id_photo);
                System.out.println("file name "+attachment);
            }
        }
        message.setContent(multipart);
    ...
    private static void addAttachmentIdCard(Multipart multipart, String filename) throws MessagingException{
        DataSource source = new FileDataSource(CommonConfig.idCardPhoto+filename);
        BodyPart messageBodyPart = new MimeBodyPart();        
        messageBodyPart.setDataHandler(new DataHandler(source));
        messageBodyPart.setFileName(filename);
        multipart.addBodyPart(messageBodyPart);
