...
<head>
    <title>OJF Survey Details for ${ojfEmployeeData[0].employee_name}</title>
    ...
    <div id="container">
        <div id="">
            <div class="container_inner">
                <p class="page_heading" style="margin-bottom: 40px;margin-left: 17px;"><span class="heading">OJF Survey Details for ${ojfEmployeeData[0].employee_name}</span> </p>
            </div>
    <!--            <p class="paging_details"><span class="heading">OJF Survey Details for ${ojfEmployeeData[0].employee_name}</span> </p>-->
            <form action="saveOJFSurveyData.htm" id="OJFSurveyForm" method="POST">
                                ...
                                <c:if test="${singleRow.answerType eq typeFreeText}">
                                    <tr>
                                        <td><label class="ojfSurveyFreeText"> ${singleRow.name}</label>
                                            <input type="hidden" name="questionId" value="${singleRow.questionId}"/>
                                            <textarea rows="2" name="answerValue${singleRow.questionId}" cols="100" style="height: 69px;width: 652px;margin: 10px;" class="required textbox_new" onchange="removeErrorDiv(this)" ${readonlyStatus}>${singleRow.empAnswer}</textarea>
                                            <div class="customError"></div>
                                        </td>
                                    </tr>
                                </c:if>
                            ...
                            <tr>
                                <td align="center" style="font-weight:bold;align:center">
                                    <c:forEach items="${surveyRatings}" var="ojfRating" >
                                        ...
                                        <input type="radio" disabled style="width:30px" name="ojf_survey_rating" value="${ojfRating.key}" ${selSurveyRating} class="required" onclick="removeErrorDivRadio(this)" > ${ojfRating.value}
                                    </c:forEach>
                                </td>
                            </tr>
                ...
                <c:if test="${exitEmpInfo.resEmpId==employeeId}">
                    <div class="buttonAlignment">
                        <div class="buttonAlignment" id="btnGroup">
                            ...
                            <input type="button" name="buttonName" id="btnCancel" value="Back" class="qualityback" onclick="javascript: location.href='${pageContext.request.contextPath}/authenticate.htm?empId=${employee_no}&modId=563'">
                        </div>
                    </div>
                </c:if>
