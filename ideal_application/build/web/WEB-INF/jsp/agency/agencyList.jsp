...
<body onload="">
    ...
    <div id="container">
        <div class="container_inner">
            <p class="page_heading" style="margin-left: 36px;"><span class="heading">Agency list</span> </p>
        </div>
...
<!--            <div class="goToList" style="width: 300px;">
                <form name="customerReportPage" id="customerReportPage" action="getCustomerXL.htm" method="POST">
                    <a class=" customerexportexcel" onclick="getExcelReport();" target="_self" style="text-decoration:none;cursor: pointer;" title="Export">Export</a>
                </form> 
                <form name="reportForm" id="reportForm" action="accrualreport.htm" method="POST">
                    <a class="add" onclick="javascript: location.href='authenticate.htm?empId=${employee_no}&modId=75'" target="_self" style="text-decoration:none;cursor: pointer;" title="Add Request">Add Agency</a>
                </form>    
            </div>-->
            <div id="datadisplay" class="formContent_new" style="width: 100%">
                <c:if test="${fn:length(agencyData)>0}">
...
<!--                        <th>
                        ...
                        <c:forEach items="${agencyData}" var="rprt" varStatus="rpritr">
...
<!--                                <td>
                                ...
                                <td>
                                    <c:forEach items="${agencyCategory}" var="agencyCategoryValues" varStatus="index" >
                                        <c:if test="${agencyCategoryValues.configKey==rprt.agencyCategory}">
                                            ${agencyCategoryValues.configValue}
                                        </c:if>
                                    </c:forEach>
                                </td>
                                <td>
                                ...
                                <td>
                                    ${rprt.status=='s'?'Submitted':'Approved'}
                                </td>
                                <td>
                                    <a title="Edit" target="_self" style="text-decoration:none" href="${pageContext.request.contextPath}/authenticate.htm?empId=${employee_no}&modId=614&agencyId=${rprt.agencyId}"><img src="${pageContext.request.contextPath}/css/images/document-blue.png" style="padding-left: 18px;"></a>
                                </td>
                            </tr>
                        </c:forEach>
                    </table>
