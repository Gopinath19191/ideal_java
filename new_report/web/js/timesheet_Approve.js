...
function filterData(prjName,fromDate,toDate)
{
...
function filterDataExport(prjName,fromDate,toDate)
{
    var DivHeight = $("html").height();
    if(prjName !="" && fromDate!="" && toDate!=""){
            $('#timesheetApproval').attr("action", "filterDataExport.htm");
            document.timesheetApproval.submit();
        }else if(prjName =="" ){
              //alert("Please select project name");
    ...
    $(":checkbox").each(function() {
        ...
        if($('#'+this.id).attr('checked')) {
            ...
            if(idValue != 'all'){
                ...
                var tDate =$('#timesheetDate_'+resId[2]).val();
               // alert("tDate "+tDate);
               var hrs = 0;
            var mins = 0;
            //old validation
//                if(appHrs != '' && appMins !=''){
//                     hrs = ($('#appHrs_'+resId[2]).val().length > 0 ? $('#appHrs_'+resId[2]).val() : '00');
//                     mins = ($('#appMins_'+resId[2]).val().length > 0 ? $('#appMins_'+resId[2]).val() : '00');
//
...
//                }
                ...
                jQuery.ajax({
                                ...
                                url: 'validateAppHrs.htm',
                                type: "POST",
                                async: false,
                                data: ({
                                    date:$('#timesheetDate_'+resId[2]).val(),
                                    empId:$('#empNumber_'+resId[2]).val()
                                ...
                                success: function(ajaxObj) { 
                                  ...
                                  var empAjax=ajaxObj.trim();
                                   stopLoading();
                                  if(empAjax === "null" || empAjax===''){
                                       // console.log("emp1 = "+empAjax);
                                       ajaxHrs=0;
                                       ajaxMins=0;
                                        return false;
                                  }else{
                                     // alert("hrs "+empAjax.split(":")[0]);
                                     // alert("mins "+empAjax.split(":")[1]);
                                      ajaxHrs=empAjax.split(":")[0];
                                      ajaxMins=empAjax.split(":")[1];
                                  }
                 ...
                 var r = $('#timesheetDate_'+resId[2]).val()+$('#empNumber_'+resId[2]).val();
                
                 if(jQuery.inArray($('#timesheetDate_'+resId[2]).val()+$('#empNumber_'+resId[2]).val(), values)== -1) {
                 var perDayCount = $(".timeClass"+r).length;
                 var countToCheckDataEntered = $(".perDayCheck_"+r+":checked").length;
                var test=$('#'+this.id).attr('checked');
                
              $("input.checkval:checked").each(function() {
                   
              var tr_id = $(this).closest("tr").attr("id");
              var value =  $("#" + tr_id + " td input.timeclassnew").attr("id");
              console.log("test--"+value);
              var commonId = (this.id).split("_")[2]; 
                $(this).parent().parent().find(".timeClass"+$('#timesheetDate_'+resId[2]).val()+$('#empNumber_'+resId[2]).val()).each(function() { 
                        
                values.push($('#timesheetDate_'+resId[2]).val()+$('#empNumber_'+resId[2]).val());
                console.log("commonId "+commonId);
                hrs += parseInt($('#appHrs_'+commonId).val() != '' ? ($('#appHrs_'+commonId).val()) : 0, 10);
                mins += parseInt($('#appMins_'+commonId).val() != '' ? ($('#appMins_'+commonId).val()) : 0, 10);
            ...
            // }  
                     ...
                     hrs=hrs+parseInt(ajaxHrs);
                     mins=mins+parseInt(ajaxMins);   
                    console.log("hrs "+hrs);
                     console.log("mis "+mins);
            
                 }
                     
                // old validation 24hrs
  ...
  c = document.querySelectorAll('.checkval');   
    if($('#'+idValue).is(':checked')){
        for (var i = 0; i < c.length; i++){
          var pName=$('#prjName_'+(i)).val(); 
          var selectedPrj=$('#selectedPrj').val();
          if(selectedPrj!=pName){
                         //c[i].checked = false; 
...
function  chkAll_Non(idValue){
  ...
  c = document.querySelectorAll('.checkval');   
    if($('#'+idValue).is(':checked')){
        ...
        for (var i = 0; i < c.length; i++){
              var pName=$('#prjNamenon_'+i).val(); 
                    if(pName=='Non_Project_Activity'){
                         //c[i].checked = false; 
                         document.querySelectorAll('.checkval')[i].checked = true; 
