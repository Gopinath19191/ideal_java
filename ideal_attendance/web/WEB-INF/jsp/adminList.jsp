<%-- 
    Document   : adminList
    Created on : 5 Dec, 2017, 12:45:17 PM
    Author     : 16221
--%>
...
<html>
    <head>
        ...
        <style type="text/css">
            ...
            .excelReport{
                background: url(images/export.png) no-repeat scroll left center transparent;
                color: #4D85B8;
                float: right;
    ...
    <body>
        <div class="container"> 
            <p class=" page_heading">Purchase Request - 
            <c:if test="${status=='e'}">
                Pending List</p>
            </c:if>
            <c:if test="${status=='c'}">
            ...
            <c:if test="${status=='e'}">
                <div class="goToList" style="width: 300px;">
                    <a class="listReport" href="procurementAdminList.htm?status=c" target="_self" style="text-decoration:none;cursor: pointer;" title="View Report">List Processed PR</a>
                    <a class="excelReport" href="procurementExcelExport.htm?status=e" target="_self" style="text-decoration:none;cursor: pointer;" title="Export Report">Export</a>
                </div>
            </c:if>
            <c:if test="${status=='c'}">
                <div class="goToList" style="width: 300px;">
                    <a class="listReport" href="procurementAdminList.htm?status=e" target="_self" style="text-decoration:none;cursor: pointer;" title="View Report">List Pending PR</a>
                    <a class="excelReport" href="procurementExcelExport.htm?status=c" target="_self" style="text-decoration:none;cursor: pointer;" title="Export Report">Export</a>
                </div>
            </c:if>
                    ...
                    <c:choose>
                        <c:when test="${fn:length(details)!=0}">
                            <c:forEach items="${details}" var="item" varStatus="i">
                                    ...
                                    <c:if test="${status=='c'}">
                                        <td><a>${item.iteam_description}</a></td>
                                    </c:if>
                                    <td style="text-align: right;"><a>${item.total}</a></td>
