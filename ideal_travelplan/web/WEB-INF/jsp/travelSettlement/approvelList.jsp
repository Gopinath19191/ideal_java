                            ...
                            <table id="datatable1" class="table display table-responsive nowrap">
                                <thead>
                                    <tr>
                                        ...
                                        <c:choose>
                                            <c:when test="${type == 'pending' && approver_type == 'finance'}">
                                                <th class="wd-10p">Submitted Date</th>
                                            </c:when>
                                            <c:otherwise>
                                        ...
                                        <c:choose>
                                            <c:when test="${type == 'pending' && approver_type == 'finance'}">
                                                <th class="wd-10p">Bill Type</th>
                                                <th class="wd-10p">Bill Received Date</th>
                                            </c:when>
                                            <c:otherwise>
                                ...
                                <tbody>
                                    <c:if test="${fn:length(settlement_details)>0}">
                                       <c:forEach items="${settlement_details}" var="adminlist" varStatus="index">
                                    <tr>
                                        <td class="wd-15p">
                                             <c:if test="${type == 'pending'}" >
                                                <a style="color: #1b84e7;" href="${pageContext.request.contextPath}/approvelSettlement.htm?ticketId=${adminlist.masterId}&approver_type=${approver_type}&type=${type}">${adminlist.ticketRefId}</a>
                                            </c:if>
                                             <c:if test="${type == 'processed'}" >
                                                <a style="color: #1b84e7;" href="${pageContext.request.contextPath}/settlementView.htm?ticketId=${adminlist.masterId}&approver_type=${approver_type}&type=${type}">${adminlist.ticketRefId}</a>
                                            </c:if>                                            
                                        </td>                                        
                                        ...
                                        <c:choose>
                                            <c:when test="${type == 'pending' && approver_type == 'finance'}">
                                                <td class="wd-10p">${adminlist.billReceivedDate}</td>
                                            </c:when>
                                        </c:choose>
                                        <td class="wd-20p">${adminlist.employeeName}</td>
                                        <td class="wd-15p">${adminlist.statusDesc}</td>
                                        <td class="wd-10p">
                                            <a href="${pageContext.request.contextPath}/settlementView.htm?ticketId=${adminlist.masterId}&approver_type=${approver_type}&type=${type}" style="color: #1b84e7;" class="icon ion-compass"><i class="fa fa-eye" style="font-size:18px"></i></a>
                                            <c:if test="${type == 'pending'}" >
                                                <a href="${pageContext.request.contextPath}/approvelSettlement.htm?ticketId=${adminlist.masterId}&approver_type=${approver_type}&type=${type}" style="color: #1b84e7;" class="icon ion-compass"><i class="fa fa-edit" style="font-size:18px; padding-left: 10px;"></i></a>
                                            </c:if>
                                        </td>
                                    </tr>
                                       </c:forEach>
