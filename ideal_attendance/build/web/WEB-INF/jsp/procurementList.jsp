<%-- 
    Document   : procurementList
    Created on : 24 Nov, 2017, 3:48:56 PM
    Author     : 16221
--%>
...
<html>
    ...
    <body>
        <div class="container"> 
            <p class=" page_heading">Purchase Request List</p>
            <div class="goToList">
                <c:if test="${approverType=='e'}">
                    <img src="/ideal_attendance/images/add.png" title="Apply Procurement" alt="Add Procurement">
                    <a style="text-decoration:none;color: #4C83B3;" target="_self" href="addProcurement.htm">Add Purchase Request</a>
                </c:if>
                <c:if test="${approverType=='r'}">
                    <img src="/ideal_attendance/images/add.png" title="Apply Procurement" alt="Add Procurement">
                    <a style="text-decoration:none;color: #4C83B3;" target="_self" href="addProcurement.htm">Add Purchase Request</a>
                </c:if>
                <c:if test="${approverType=='b'}">
                    <img src="/ideal_attendance/images/add.png" title="Apply Procurement" alt="Add Procurement">
                    <a style="text-decoration:none;color: #4C83B3;" target="_self" href="addProcurement.htm">Add Purchase Request</a>
                </c:if>
            </div>
            <div align="center" style="margin:15px 0px -20px 0px;" id="successDiv">
                <font color="green" size="4">${successMessage}</font>
                        ...
                        <c:when test="${fn:length(details)!=0}">
                            <c:forEach items="${details}" var="item" varStatus="i">
                                    ...
                                    <td align="center">
                                        ...
                                        <c:if test="${approverType=='e'}">
                                            <c:if test="${item.status == 's' || item.status == 'r' || item.status == 'j'}">
                                                <a href="editProcurement.htm?pp_id=${item.id}"><img src="${pageContext.request.contextPath}/images/edit-icon.png" /></a>
                                          </c:if> 
                                        </c:if>
                                    </td>
                                </tr>
