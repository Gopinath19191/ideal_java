...
$(document).ready(function()
{
    ...
    <script type="text/javascript">
     ...
     function setAppraiseeData(qpdId,appraiseeId,bandId,appraisalQuarter,appraisalYear,appraiserName,appraiseeEmpId,departmentId,reviewerMenuCheck){
       //  alert(appraiseeId+appraiserName+appraisalQuarter+appraisalYear)
     
         $("#qpdIdSelected").val(qpdId);
...
<body onload="changeTabClass('aaReviewer');" style="width: auto;" >
    ...
    <form action="reviewerAction.do" method="POST" name="saveReview" id="saveReview">
              ...
              <div class="contentarea">
                  <div class="qpdSearch" style="width: 97%">
                      <table id="apprMaster" cellpadding="2" cellspacing="0" align="center" border="0" style="width: 80%">
                    <tr>
                        <td width="36%" style="float: right;">Appraiser</td>
                            <td width="25%"> <select name="myappraiserId" class="selectbox" style="width: 200px;" onchange="filterByAppraiser()">
                            ...
                            <td width="13%">Appraisal Year</td><td>
                            <select name="appraisalYear" id="appraisalYear" class="required selectbox" style="width: 5em" onchange="filterByAppraiser()" >
...
<%--<c:out value="${reviewerDisabled}-${appraiseeCurrentStatus}"></c:out>--%>
                                            ...
                                            <tbody>
                                            <tr class="${rowDisplay}">
                                                <td><a style="color: #4C83B3;font-size: 12px;font-weight: bold" href="javascript:void(0)" onclick="setAppraiseeData(${tes.qpdId},${tes.appraiseeId},${tes.bandId},${appraisalQuarter},${appraisalYear},'${tes.appraiserName}',${tes.appraiseeEmpId},${tes.departmentId},'reviewer')">${tes.appraiseeEmpId}</a>
                                                    <input type="hidden" value="${tes.appraiseeId}" name="appraiseeIdForm[]" ${reviewerDisabled}>
                                                     <input type="hidden" value="${tes.qpdId}" name="appraiseeIdForm[]" ${reviewerDisabled}>
                                                ...
                                                <td colspan="2" style="text-align: center;">
                                                    ...
                                                    <td colspan="2" width="100px"><span class="displayText">
                                                            <c:choose>
                                                                <c:when test="${tes.appraiserPromotionRecommeded!='' && tes.appraiserPromotionRecommeded!=null && tes.appraiserPromotionRecommeded==1}">
                                                                    <c:set value="Yes" var="appPromoRec" />
                                                                </c:when>
                                                                <c:when test="${tes.appraiserPromotionRecommeded!='' && tes.appraiserPromotionRecommeded!=null && tes.appraiserPromotionRecommeded==0}">
                                                    ...
                                                    <td colspan="2" width="100px">
                                                        ...
                                                        <c:set value="" var="appPromStatus" />
                                                        <c:choose>
                                                            <c:when test="${tes.reviewerPromotionRec!='' && tes.reviewerPromotionRec!=null && tes.reviewerPromotionRec==1}">
                                                                <c:set value="selected" var="appPromStatusYes" />
                                                            </c:when>
                                                            <c:when test="${tes.reviewerPromotionRec!='' && tes.reviewerPromotionRec!=null && tes.reviewerPromotionRec==0}">
                                                        ...
                                                        <select style="width: 85px;text-align: center;" name="reviewerPromotionRecommended[]" class="required selectbox" onchange="removeErrorDiv(this.id,'Select Promotions');" id="${tes.appraiseeEmpId}" ${reviewerDisabled}>
                                                  ...
                                                  <td>
                                                       <a class="qpdRating" style="color: #4C83B3;font-size: 12px;font-weight: bold" href="${pageContext.request.contextPath}/com/defiance/ideal/qpd/managers/getQpdRating.do?appraiseeId=${tes.appraiseeId}" rel="${pageContext.request.contextPath}/com/defiance/ideal/qpd/managers/lastThreeQpdRatings.jsp">Previous QPD</a>
                                                  </td>
                                                    <td>
                                                                <select style="width: 47px;text-align: center;" name="reviewerRatingForm[]" class="reviewerRatings selectbox" onchange="calculateRatings();removeErrorDiv(this.id,'Select Ratings');" id="${tes.appraiseeEmpId}" ${reviewerDisabled}>
                                                                    <option value="">-- Select Rating --</option>
                                                                        <c:forEach begin="1" end="5" var="rateValue">
                                                                            <c:choose>
                                                                                <%--<c:when test="${tes.appraiserRating == rateValue && tes.reviewerRating<1}">--%>
                                                                                <c:when test="${tes.appraiserRating == rateValue && (tes.reviewerRating==null || tes.reviewerRating=='' || tes.reviewerRating==0)}">
                                                                                    <option value="${rateValue}" selected="selected">${rateValue}</option>
                                                                                </c:when>
                                                                                <%--<c:when test="${tes.reviewerRating == rateValue && tes.reviewerRating>0}">--%>
                                                                                <c:when test="${tes.reviewerRating == rateValue && (tes.reviewerRating!=null || tes.reviewerRating!='' || tes.reviewerRating!=0)}">
                                        ...
                                        <table width="100%">
                                               <tr>
                                                   <td style="text-align: center">
                                                       <input class="buttons chartbutton" type="button" id="showChart" onclick="showHideChart(this)" value="Show Chart" style="display:none"/>
                    ...
                    <table width="100%">
                        <tr><td style="text-align: center;"><input type="submit" name="button" id="saveRating" class="buttons exportbutton" value="Export to Excel"  />
                                <c:if test="${submitCount >= 1 && reviewerSubmitCount < appraiseeCount && buttonsDisplay=='show'}">
                                    ...
                                    <input style="width: 199px;" type="submit" name="submitToNormalizer" id="SubmitToNormalizer" class="buttons submitbutton" value="Submit for Final Review" onclick="return validateReviewerComments()" />
            ...
            <form id="appraiseeInfo" name="appraiseeInfo" action="" method="post">
                                    <input type="hidden" name="appraiseeIdSelected" id="appraiseeIdSelected">
                                     <input type="hidden" name="qpdIdSelected" id="qpdIdSelected">
