...
//        mailPassword="Passw0rd@123";
    ...
    public boolean smtpMail(final String mailTo, final String mailSubject, final String mailMessage, final String mailCC,final String mailBcc, String empId, String fileName)
			throws Exception {
                System.out.println(mailTo+mailSubject+mailMessage+mailCC+" forwarded details from insertExitEmpDetails"+mailBcc);
...
//        multipart.addBodyPart(messageBodyPart);
        ...
        if("Reg: PF related notes".equals(mailSubject)){
            multipart.addBodyPart(attachFile1());
            multipart.addBodyPart(attachFile2());
            //multipart.addBodyPart(attachFile3(request,response,fileName));
        }
        String ressub="Acceptance of Resignation Letter - "+empId;
        if(ressub.equals(mailSubject)){
            multipart.addBodyPart(attachFile3(fileName));
        }
        String sersub="Service Certificate and Relieving Letter - "+empId;
        if(sersub.equals(mailSubject)){
            multipart.addBodyPart(attachFile3(fileName));
        }
    ...
    public MimeBodyPart attachFile1() throws MessagingException{
         messageBodyPart = new MimeBodyPart();
         String filename = CommonConfigurations.PF_Guideliness;
         DataSource source = new FileDataSource(filename);
         messageBodyPart.setDataHandler(new DataHandler(source));
         messageBodyPart.setFileName(source.getName());
         return messageBodyPart;
    }
    public MimeBodyPart attachFile2() throws MessagingException{
         messageBodyPart = new MimeBodyPart();
         String filename = CommonConfigurations.PF_ProcessFlow;
         DataSource source = new FileDataSource(filename);
         messageBodyPart.setDataHandler(new DataHandler(source));
         messageBodyPart.setFileName(source.getName());
         return messageBodyPart;
    }
   public MimeBodyPart attachFile3(String fileName) throws MessagingException, Exception{
         messageBodyPart = new MimeBodyPart();
         DataSource source = new FileDataSource(fileName);
         messageBodyPart.setDataHandler(new DataHandler(source));
